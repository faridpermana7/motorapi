...
class UserRepository:
    ...
    async def create_user(self, data: UserDTO) -> Optional[UserResponseDTO]: 
        entity = UserEntity(
            **data.dict(
                exclude={
                    "password", 
    ...
    async def update_user(self, user_id: int, data: UserDTO) -> Optional[UserResponseDTO]:
        result = await self.session.execute(select(UserEntity).where(UserEntity.id == user_id))
        entity = result.scalars().first()
        if not entity:
            return None
    ...
    async def delete_soft_user(self, user_id: int) -> bool:
        result = await self.session.execute(select(UserEntity).where(UserEntity.id == user_id))
        entity = result.scalars().first()
        if not entity:
            return False
...
class UserService:
    ...
    async def delete_soft_user(self, user_id: int) -> bool:
        return await self.repo.delete_soft_user(user_id)
